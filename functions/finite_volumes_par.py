...
def temporal_loop_par(initial_phi, damage):
    ...
    for ti in np.arange(ntimes):
        ...
        phi_r = np.reshape(phi[:,ti],(n,n)).copy()
        
        # Row by row
        for j in range(n):
            
            phi_r[j,:]=optimize.fsolve(lambda phi_rplus1: spatial_discretization_row(phi_r, phi_rplus1, phi_0[j,:], n, dx, dt, epsilon, lam[j,:], j), phi_r[j,:])
        ...
        for j in range(n):
            ...
            phi_c[:,j] = optimize.fsolve(lambda phi_cplus1: spatial_discretization_column(phi_c, phi_cplus1, phi_0[:,j], n, dx, dt, epsilon, lam[:,j], j), phi_c[:,j])
            

        phi[:,ti+1] = np.reshape(phi_c,n*n).copy()
...
def spatial_discretization_row(phi_r, phi_rplus1, phi_0, n, dx, dt, epsilon, lam, j):
    ...
    if j==0:
        ...
        Lap[1:-1]=epsilon**2/dx**2/2.*(phi_r[j,2:]-2*phi_r[j,1:-1]+phi_r[j,0:-2]\
           ...
           +phi_rplus1[2:]-2*phi_rplus1[1:-1]+phi_rplus1[0:-2]\
           +phi_r[j+1,1:-1]-2*phi_rplus1[1:-1]+phi_r[j-1,1:-1])
       
        Lap[0]=epsilon**2/dx**2/2.*(phi_r[j,1]-phi_r[j,0]\
           +phi_r[j+1,0]-2*phi_r[j,0]+phi_r[j-1,0]\
           +phi_rplus1[1]-phi_rplus1[0]\
           +phi_r[j+1,0]-2*phi_rplus1[0]+phi_r[j-1,0])
        
        Lap[-1]=epsilon**2/dx**2/2.*(-phi_r[j,-1]+phi_r[j,-2]\
           +phi_r[j+1,-1]-2*phi_r[j,-1]+phi_r[j-1,-1]\
           -phi_rplus1[-1]+phi_rplus1[-2]\
           +phi_r[j+1,-1]-2*phi_rplus1[-1]+phi_r[j-1,-1])
    ...
    Fhalf[1:-1]=uhalfplus*mobility(phi_rplus1[:-1])+uhalfminus*mobility(phi_rplus1[1:]) 

    # Compute residual of implicit finite-volume scheme

    E_i = phi_rplus1-phi_r[j,:]+(Fhalf[1:]-Fhalf[:-1])*dt/dx + lam * (phi_0 - phi_rplus1) * dt
...
def spatial_discretization_column(phi_c, phi_cplus1, phi_0, n, dx, dt, epsilon, lam, j):
    ...
    if j==0:
        ...
        Lap[1:-1]=epsilon**2/dx**2/2.*(phi_c[2:,j]-2*phi_c[1:-1,j]+phi_c[0:-2,j]\
           ...
           +phi_cplus1[2:]-2*phi_cplus1[1:-1]+phi_cplus1[0:-2]\
           +phi_c[1:-1,j+1]-2*phi_cplus1[1:-1]+phi_c[1:-1,j-1])
        
        Lap[0]=epsilon**2/dx**2/2.*(phi_c[1,j]-phi_c[0,j]\
           +phi_c[0,j+1]-2*phi_c[0,j]+phi_c[0,j-1]\
           +phi_cplus1[1]-phi_cplus1[0]\
           +phi_c[0,j+1]-2*phi_cplus1[0]+phi_c[0,j-1])
        
        Lap[-1]=epsilon**2/dx**2/2.*(-phi_c[-1,j]+phi_c[-2,j]\
           +phi_c[-1,j+1]-2*phi_c[-1,j]+phi_c[-1,j-1]\
           -phi_cplus1[-1]+phi_cplus1[-2]\
           +phi_c[-1,j+1]-2*phi_cplus1[-1]+phi_c[-1,j-1])
    ...
    Fhalf[1:-1]=uhalfplus*mobility(phi_cplus1[:-1])+uhalfminus*mobility(phi_cplus1[1:]) 

    # Compute residual of implicit finite-volume scheme

    E_i=phi_cplus1-phi_c[:,j]+(Fhalf[1:]-Fhalf[:-1])*dt/dx + lam * (phi_0 - phi_cplus1) * dt
